...
@section('content')
    <div>
        <h1 class="sans text-xl text-extrabold">Table Apa Saja Yang Diukur</h1>
    </div>
    ...
    <div class="mt-4 relative overflow-x-auto shadow-lg sm:rounded-lg">
        <div class="my-4">
            <a href="{{ url('dashboard/tambah-pengukuran') }}"
                class="md:float-right py-2 px-3 bg-blue-950 text-white hover:opacity-75  text-base font-medium text-center border border-gray-300 rounded-lg  focus:ring-4 focus:ring-gray-100">
                Tambah Data
            </a>
        </div>
                ...
                @forelse ($data as $item)
                    <tr
                        ...
                        <td class="px-6 py-4 flex ">
                            <a href="{{ url('dashboard/edit-pengukuran/' . Crypt::encrypt($item->id)) }}"
                                class="font-medium text-green-600 mr-2">
                                <i class="fas fa-pencil"></i>
                            </a>
                            <form onsubmit="return confirm('Apakah anda yakin ingin menghapus data ini ?');"
                                action="{{ url('dashboard/delete-pengukuran/' . Crypt::encrypt($item->id)) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
