...
@section('content')
    <div>
        <h1 class="sans text-xl text-extrabold">Table Jadwal</h1>
    </div>
                ...
                @forelse ($data as $item)
                    <tr
                        ...
                        <td class="px-6 py-4 flex ">
                            <a href="{{ url('dashboard/edit-jadwal/' . Crypt::encrypt($item->id)) }}"
                                class="font-medium text-green-600 mr-2"> <i class="fas fa-pencil"></i>
                            </a>
                            <form onsubmit="return confirm('Apakah anda yakin ingin menghapus data ini ?');"
                                action="{{ url('dashboard/delete-jadwal/' . Crypt::encrypt($item->id)) }}" method="POST">
                                @csrf
                                @method('DELETE')
